...
try:
    from db.database import get_database_manager as get_firebase_db, DatabaseManager as FirebaseDB
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "version": "2.0.0",
        "database": firebase_db is not None,
...
async def create_post(post: PostCreate):
    ...
    success = firebase_db.save_post(post_data)
    if not success:
...
# ==================== Scraper & Pages.txt Control Endpoints ====================

import subprocess
import sys

# Global process tracker for scraper
scraper_process = None

class PagesUpdateRequest(BaseModel):
    pages: List[str]

@api_v1.get("/pages", response_model=Dict[str, Any])
async def get_pages():
    """Read pages.txt file"""
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to read pages.txt: {str(e)}")

@api_v1.post("/pages", response_model=Dict[str, Any])
async def update_pages(request: PagesUpdateRequest):
    """Overwrite pages.txt file"""
    pages_file = os.path.join(os.path.dirname(__file__), "pages.txt")
    try:
        with open(pages_file, "w", encoding="utf-8") as f:
            f.write("# Pages to scrape (auto-generated by UI)\n")
            for page in request.pages:
                f.write(f"{page}\n")
        return {"status": "success", "message": "pages.txt updated successfully"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to write pages.txt: {str(e)}")

@api_v1.post("/scraper/run", response_model=Dict[str, Any])
async def run_scraper_endpoint(background_tasks: BackgroundTasks):
    """Trigger the Facebook scraper in the background"""
    ...
    def start_scraper():
        ...
        try:
            ...
            # Spawn the scraper headless, writing output straight to DEVNULL since it writes to log file
            scraper_process = subprocess.Popen(
                [python_executable, script_path, "--headless"],
                cwd=os.path.dirname(__file__),
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL
            )
        except Exception as e:
            print(f"Failed to start scraper in background: {e}")

    background_tasks.add_task(start_scraper)
    return {"status": "success", "message": "Scraper started in background"}

@api_v1.get("/scraper/status", response_model=Dict[str, Any])
async def get_scraper_status():
    """Check if the scraper is currently active"""
    global scraper_process
    is_active = scraper_process is not None and scraper_process.poll() is None
    return {
        "status": "success",
        "active": is_active,
        "pid": scraper_process.pid if is_active and scraper_process else None
    }

@api_v1.get("/scraper/logs", response_model=Dict[str, Any])
async def get_scraper_logs(lines: int = Query(default=150, le=1000)):
    """Fetch the latest N lines of scraper logs"""
    log_file = os.path.join(os.path.dirname(__file__), "logs", "facebook_scraper.log")
    try:
        if not os.path.exists(log_file):
            return {"status": "success", "logs": "No logs recorded yet. Start the scraper to generate logs."}
